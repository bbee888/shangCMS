...
class Database extends Common
{
    ...
    public function index($group = 'export')
    {
        ...
        if ($group == 'export') {
            ...
            //列出备份文件列表
            $path = trim(config('databases.database_path'), '/').DS;
            if (!is_dir($path)) {
                Dir::create($path);
            }
    ...
    public function export($id = '', $start = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $config = array(
                'path'     => trim(config('databases.database_path'), '/').DS,
                'part'     => config('databases.database_size'),
                'compress' => config('databases.database_compress'),
                'level'    => config('databases.database_level'),
            );
    ...
    public function import($id = '')
    {
        ...
        $path  = trim(config('databases.database_path'), '/').DS.$name;
        $files = glob($path);
        $list  = array();
